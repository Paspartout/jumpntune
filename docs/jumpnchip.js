...
var ASM_CONSTS = [function($0) { if ((window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(mal) === 'undefined') { mal = {}; mal.devices = []; mal.track_device = function(device) { for (var iDevice = 0; iDevice < mal.devices.length; ++iDevice) { if (mal.devices[iDevice] == null) { mal.devices[iDevice] = device; return iDevice; } } mal.devices.push(device); return mal.devices.length - 1; }; mal.untrack_device_by_index = function(deviceIndex) { mal.devices[deviceIndex] = null; while (mal.devices.length > 0) { if (mal.devices[mal.devices.length-1] == null) { mal.devices.pop(); } else { break; } } }; mal.untrack_device = function(device) { for (var iDevice = 0; iDevice < mal.devices.length; ++iDevice) { if (mal.devices[iDevice] == device) { return mal.untrack_device_by_index(iDevice); } } }; mal.get_device_by_index = function(deviceIndex) { return mal.devices[deviceIndex]; }; } return 1; },
 ...
 function($0) { return mal.get_device_by_index($0).webaudio.sampleRate; },
 function($0, $1) { var device = mal.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; if (device.intermediaryBuffer !== undefined) { Module._free(device.intermediaryBuffer); device.intermediaryBuffer = undefined; device.intermediaryBufferView = undefined; device.intermediaryBufferSizeInBytes = undefined; } mal.untrack_device_by_index($0); },
 function($0) { mal.get_device_by_index($0).webaudio.resume(); },
 function($0) { mal.get_device_by_index($0).webaudio.suspend(); },
 function($0) { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); }];
...
var real__mal_device_process_pcm_frames__webaudio = asm["_mal_device_process_pcm_frames__webaudio"]; asm["_mal_device_process_pcm_frames__webaudio"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return real__mal_device_process_pcm_frames__webaudio.apply(null, arguments);
};
...
var _mal_device_process_pcm_frames__webaudio = Module["_mal_device_process_pcm_frames__webaudio"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["_mal_device_process_pcm_frames__webaudio"].apply(null, arguments) };
